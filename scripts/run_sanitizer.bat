...
if "%1"=="memcheck" (
    ...
    compute-sanitizer --tool memcheck --target-processes all %OBJ_DIR%\cuNBSim.exe
) else if "%1"=="racecheck" (
    echo Ejecutando racecheck...
    compute-sanitizer --tool racecheck --target-processes all %OBJ_DIR%\cuNBSim.exe
) else if "%1"=="initcheck" (
    echo Ejecutando initcheck...
    compute-sanitizer --tool initcheck --target-processes all %OBJ_DIR%\cuNBSim.exe
) else if "%1"=="synccheck" (
    echo Ejecutando synccheck...
    compute-sanitizer --tool synccheck --target-processes all %OBJ_DIR%\cuNBSim.exe
) else (
